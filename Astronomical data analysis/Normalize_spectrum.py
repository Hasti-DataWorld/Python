...
def ontype(event):
    ...
    # spectrum
        ...
        if continuum is not None:
            plt.cla()
            #dataout = Table([wave, flux, err, continuum, flux/continuum], names=['lambda', 'flux', 'err','continuum' 'normflux'])
            #ascii.write(dataout, os.path.splitext(filename)[0]+'.norm', overwrite=True)
            plt.plot(wave,flux/continuum,'k-',label='normalised')

    # when the user hits 'r': clear the axes and plot the original spectrum
    elif event.key=='r':
        plt.cla()
        plt.plot(wave,flux,'k-', linewidth=0.5)
...
if __name__ == "__main__":
    ...
    filename = sys.argv[1]
    wave,flux= np.loadtxt(filename, usecols=(0,1)).T
    spectrum, = plt.plot(wave,flux,'k-',label='spectrum')
    plt.title(filename)
    plt.ylim(np.min(flux), 9e-14)    ## I added these
