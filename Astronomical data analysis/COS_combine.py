"""
Co-adding Hubble Space Telescope COS x1d files and return a COSSpectrum 
...
def coadd(files=None, path='.', chan=1, method=1,
    ...
    gratingb=grating
    grating_list = []
    for item in gratingb:
        grating_list.append(str(item,'UTF-8'))

    grating=np.array(grating_list)

    print(grating)
    ####################################

    if type(chanind) == int:
        g = np.where(np.char.strip(grating, 'b') == channame[chanind])
        g = g[0]
    else:
        g = set(np.where(np.char.strip(grating, 'b') == channame[chanind[0]])[0]).union(
            set(np.where(np.char.strip(grating, 'b') == channame[chanind[1]])[0]))
        g = np.array(list(g))
    ...
    spec = fits.open("".join((path,files[0])))
    ...
    for n, i in enumerate(files):
        ...
        for j in range(2):
            for k in range(nwave):
                ...
                aa = np.where(spec[1].data[j][4][int(lo):int(hi) + 1] == 0.0) # I added int function here, because of slicing, and needs for int instead of fload
                naa = len(aa[0])
                zeroes[k] = naa
    ...
    if nfiles > 1 and no_align == False:
        ...
        for i in chanind_dum:
            ...
            ref = ref[0]
            filenote[ref] = filenote[ref].decode() + "*"
    ...
    if verbose:
        ...
        for i in range(nfiles):
            print(str(i) + "-" + " ".join((str(files[i]),
                                           ...
                                           str(dateobs[i], 'UTF-8'),  # I added 'UTF-8' here to drop the 'b' in the format, but is not nesessary
                                           str(timeobs[i], 'UTF-8'),
                                           str(filenote[i], 'UTF-8'))))
    ...
    if savefile != None and savefile != False:
        ...
        if saveformat == "dat":
            f = open(fname + ".dat", 'w') # I made a change here, I doped the b in 'wb', because of string and btype format issues. "wb" works in python2 
